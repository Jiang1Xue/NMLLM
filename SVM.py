...
def trasform_data_format(data, label):
    ...
    # 创建空的多维数组用于存放数据
    dataset_array = np.zeros(shape=(c, r))
    # 创建空的数组用于存放图片的标注信息
    ...
    for i in range(c):
        # 格式转化为148x148x1shape
        data_reshaped = np.reshape(data[:, i], newshape=(1, r))
        # 将维度转换后的图片存入指定的数组内
...
def main():
    ...
    for train_index, test_index in ss.split(isoform_express_scaled_state, sample_label_state):
        X_train, X_test = isoform_express_scaled_state[train_index], isoform_express_scaled_state[test_index]
        y_train, y_test = sample_label_state[train_index], sample_label_state[test_index]
        y_test_true = sample_label_state[test_index]
        sample_test_name = sample_name[test_index]
        clf = SVC(C=1.0, cache_size=200, class_weight=None, coef0=0.0, decision_function_shape='ovr', degree=3, gamma='auto', kernel='rbf', max_iter=-1)
    ...
    predict_label_s = np.reshape(predict_label_s, (-1, 1))
    print(predict_label_s.shape)
    ...
    col_names = ['sample name', 'true_label', 'predict', 'predict']
    ...
    start_time_all = time.clock()
    for train_index, test_index in ss.split(isoform_express_scaled_cognitive, sample_label_cognitive):
        X_train, X_test = isoform_express_scaled_state[train_index], isoform_express_scaled_cognitive[test_index]
        y_train, y_test = sample_label_cognitive[train_index], sample_label_cognitive[test_index]
        y_test_true = sample_label_cognitive[test_index]
        sample_test_name = sample_name[test_index]
        clf = SVC(C=1.0, cache_size=200, class_weight=None, coef0=0.0, decision_function_shape='ovo', degree=3, gamma='auto', kernel='rbf', max_iter=-1)
        clf.fit(X_train, y_train)
        predict = clf.predict(X_test)
    ...
    predict_label_c = np.reshape(predict_label_c, (-1, 1))
    ...
    # 保存结果
    cost = [cost_all_s, cost_all_c]
    cost = np.array(cost)
    cost_df = pd.DataFrame(data=cost)
    cost_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/svm/10/time_cost.csv')

    label_all_s_df = pd.DataFrame(data=label_all_s)
    label_all_c_df = pd.DataFrame(data=label_all_c)

    label_all_s_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/svm/10/label_all_s.csv')
    label_all_c_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/svm/10/label_all_c.csv')
