...
def Perceptron_2():
    model = Sequential()
    model.add(Flatten(input_shape=(1, 6180)))
    ...
    optimizer = Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=1e-8)
...
def trasform_data_format(data, label):
    ...
    # 创建空的多维数组用于存放数据
    dataset_array = np.zeros(shape=(c, 1, r))
    # 创建空的数组用于存放图片的标注信息
    ...
    for i in range(c):
        # 格式转化为148x148x1shape
        data_reshaped = np.reshape(data[:, i], newshape=(1, 1, r))
        # 将维度转换后的图片存入指定的数组内
...
def main():
    ...
    isoform_express_scaled_cognitive, sample_label_cognitive = trasform_data_format(isoform_express_scaled, sample_label_cognitive)
    # 对标签数据进行OneHot编码
    ...
    col_names = ['sample name', 'true_label', 'predict 0', 'predict 1', 'predict 2', 'predict 3', 'predict 4', 'predict 5', 'predict']
    col_names = np.array(col_names)
    ...
    cost_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/perceptron/10/time_cost.csv')

    label_all_s_df = pd.DataFrame(data=label_all_s)
    label_all_c_df = pd.DataFrame(data=label_all_c)

    label_all_s_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/perceptron/10/label_all_s.csv')
    label_all_c_df.to_csv('/Users/xuejiang/PycharmProjects/isofom_/result/perceptron/10/label_all_c.csv')
